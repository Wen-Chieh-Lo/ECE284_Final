...
class ResNet_Cifar_2bit(nn.Module):

    def __init__(self, block, layers, num_classes=10, float=False):
        super(ResNet_Cifar_2bit, self).__init__()
        self.inplanes = 16
        # self.conv1 = nn.Conv2d(3, 16, kernel_size=3, stride=1, padding=1, bias=False)
        self.conv1 = QuantConv2d_2bit(3, 16, kernel_size=3, stride=1, bias=False)
        self.bn1 = nn.BatchNorm2d(16)
        self.relu = nn.ReLU(inplace=True)
        self.layer1 = self._make_layer(block, 16, layers[0], float=float)
        self.layer2 = self._make_layer(block, 32, layers[1], stride=2, float=float)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, float=False):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                QuantConv2d_2bit(self.inplanes, planes * block.expansion, kernel_size=1, stride=stride, bias=False)
                if float is False else nn.Conv2d(self.inplanes, planes * block.expansion, kernel_size=1,
                                                 stride=stride, bias=False),
                nn.BatchNorm2d(planes * block.expansion)
            )
...
def resnet20_quant_2bit(**kwargs):
    model = ResNet_Cifar_2bit(BasicBlock, [3, 3, 3], **kwargs)
    return model
