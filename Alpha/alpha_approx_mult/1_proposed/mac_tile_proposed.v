...
module mac_tile_proposed (clk, out_s, in_w, out_e, in_n, inst_w, inst_e, reset, sel_mode, psum_val);
...
output [psum_bw-1:0] psum_val; // just for flush operation
...
// for psum en signal 
wire c_en_temp;
FF #(.bw(1), .rst_val(1'b0)) FF_en_c (.clk(clk), .reset(reset), .en(1'b1), .d(inst_w[1]), .q(c_en_temp));

// for psum ff
wire c_en; assign c_en = (sel_mode==1'b0) ? 1'b1 : c_en_temp;
...
mac_proposed #(.bw(bw), .psum_bw(psum_bw)) mac_instance (
        .a(a_q), 
        .b(b_q),
        .c(c_q),
	.out(mac_out)
