...
from mult import proposed_approx_mul
...
def approx_conv2d(self, x_int, w_int, padding, stride):

    batch_size, in_channels, height, width = x_int.shape
    out_channels, _, kernel_h, kernel_w = w_int.shape
    
    out_height = int((height + 2 * padding - kernel_h) / stride + 1)
    out_width = int((width + 2 * padding - kernel_w) / stride + 1)

    output = torch.zeros((batch_size, out_channels, out_height, out_width), device=x_int.device)
    x_padded = F.pad(x_int, (padding, padding, padding, padding))
                    ...
                    for ic in range(in_channels):
                        for kh in range(kernel_h):
                            for kw in range(kernel_w):
                                val_x = int(x_padded[b, ic, h_start + kh, w_start + kw].item())
                                val_w = int(w_int[oc, ic, kh, kw].item())                                
                                product = proposed_approx_mul(val_x, val_w)
                                acc += product
...
class QuantConv2d(nn.Conv2d):
    ...
    def forward(self, x):
        ...
        x_bit = format(x, '08b')
        weight_bit = format(weight_q, '08b')
        return approx_conv2d(x_bit, weight_bit, self.stride, self.padding)
